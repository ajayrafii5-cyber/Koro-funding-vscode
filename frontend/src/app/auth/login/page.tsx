...
export default function LoginPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const redirect = localStorage.getItem("redirectAfterLogin"); localStorage.removeItem("redirectAfterLogin"); router.push(redirect ?? (data.trader?.role === "ADMIN" ? "/admin" : "/dashboard"));
    } catch {
    ...
    <div className="min-h-screen bg-bg text-white flex items-center justify-center px-6">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <h1 className="font-heading font-extrabold text-3xl mb-2">Masuk</h1>
          <p className="text-muted text-sm">Belum punya akun? <Link href="/auth/register" className="text-teal hover:underline">Daftar sekarang</Link></p>
        </div>
        <div className="bg-surface border border-white/5 rounded-2xl p-6 space-y-4">
          ...
          <div>
            <div className="flex justify-between items-center mb-2"><label className="text-xs text-muted uppercase tracking-wider">Password</label><Link href="/auth/forgot-password" className="text-xs text-teal hover:underline">Lupa Password?</Link></div>
